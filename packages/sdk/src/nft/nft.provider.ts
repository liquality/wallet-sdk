...
import { InfuraNftProvider } from "./providers/infura-nft-provider";
import { Nft, NftsForContract, NftType } from "./types";

export abstract class NftProvider extends BaseNftProvider {
  private static nftProviders: BaseNftProvider[] = [
    AlchemyNftProvider,
    InfuraNftProvider,
  ];
  ...
  public static async getNftsForContract(
    ...
    for (let i = 0; i < this.nftProviders.length && !nfts!; i++) {
      nfts = await this.nftProviders[i].getNftsForContract(
        contractAddress,
        chainID,
        options
      );
