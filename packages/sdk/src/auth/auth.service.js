...
const service_provider_torus_1 = __importDefault(require("@tkey/service-provider-torus"));
const storage_layer_torus_1 = __importDefault(require("@tkey/storage-layer-torus"));
const security_questions_1 = __importDefault(require("@tkey/security-questions"));
...
class AuthService {
    ...
    async generateNewShareWithPassword(tKey, password) {
        if (password.length > 10) {
            const result = await tKey.modules.securityQuestions.generateNewShareWithSecurityQuestions(password, "whats your password?");
    ...
    async inputShareFromSecurityQuestions(tKey, password) {
        ...
        if (password.length > 10 && tKey) {
            try {
                ...
                await tKey.modules.securityQuestions.inputShareFromSecurityQuestions(password);
