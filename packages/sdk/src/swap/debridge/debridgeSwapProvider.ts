...
export abstract class DebridgeSwapProvider {
    ...
    public static async swap(swapRequest: SwapRequest, pk: string) {
        ...
        if (new BigNumber(swapRequest.srcChainTokenInAmount).lte(0) || !isSrcChainSupported || !isDstChainSupported)
        ...
        if(swapRequest.srcChainTokenIn !== AddressZero) {
            // Approve token
          const contract: ERC20 = ERC20__factory.connect(AddressZero, chainProvider).attach(swapRequest.srcChainTokenIn);
          const decimals = await contract.decimals();

          const fromAmountInUnits = parseUnits(swapRequest.srcChainTokenInAmount, decimals).toString();

          await this.approveToken(contract, fromAmountInUnits, wallet);
        }else{
          fromAmountInUnits = parseEther(swapRequest.srcChainTokenInAmount).toString();
        }
    ...
    private static async singleChainSwap(swapRequest: SwapRequest, fromAmountInUnits: string, wallet: Wallet)  {
      const {srcChainId: chainId,srcChainTokenIn: tokenIn, srcChainTokenInAmount: tokenInAmount,dstChainTokenOut: tokenOut,dstChainTokenOutRecipient: tokenOutRecipient} = swapRequest;
      ...
      const swapConfirmation =  await this.waitForSingleChainSwapConfirmations(swapTx.hash, chainId);
  ...
  private static async crossChainSwap(swapRequest: SwapRequest, fromAmountInUnits: string, wallet:Wallet)  {
    const {srcChainId,srcChainTokenIn,srcChainTokenInAmount,dstChainId,dstChainTokenOut,dstChainTokenOutRecipient} = swapRequest;
    const tx =  await axios({
    ...
    const sendConfirmation =  await this.waitForCrossChainSendConfirmations(sendTx.hash, srcChainId);
    if(sendConfirmation.status === 'FAILED') return false; 
    const receiveConfirmation = await this.waitForCrossChainReceiveConfirmations(sendTx.hash);
  ...
  private static async approveToken(contract: ERC20, amount: string, wallet: Wallet) {

    if ((await this.hasAllowance(contract, amount, wallet.address))) return true;


    const tx = await contract.populateTransaction.approve(this.config.routerAddress, amount);
    const preparedTx = await TransactionService.prepareTransaction({
      ...tx,
      to: contract.address,
  ...
  private static async hasAllowance(contract: ERC20, amount: string, fromAddress: string): Promise<boolean> {
    const allowance = await contract.allowance(fromAddress, this.config.routerAddress);
    return allowance.gte(amount);
  }

  private static async waitForApproveConfirmations(approveTxHash: string, chainProvider: ethers.providers.Provider) {
  ...
  private static async waitForSingleChainSwapConfirmations(swapTxHash: string, chainId: number) {
    const chainProvider  = getChainProvider(chainId);

    return withInterval(async () => {
      try{
        const tx  = await chainProvider.getTransaction(swapTxHash);
        if (tx.confirmations && tx.confirmations > this.config.chains[chainId].minBlockConfirmation){
            const { status } = await chainProvider.getTransactionReceipt(swapTxHash);
            if (Number(status) === 1) {
                return {
                  status: 'SUCCESS',
                };
  ...
  private static async waitForCrossChainSendConfirmations(sendTxHash: string, chainId: number) {
      const chainProvider  = getChainProvider(chainId);
      ...
      return withInterval(async () => {
        try{
          ...
          if (tx.confirmations && tx.confirmations > this.config.chains[chainId].minBlockConfirmation){
              const { status } = await chainProvider.getTransactionReceipt(sendTxHash);
              if (Number(status) === 1) {
  ...
  private static async waitForCrossChainReceiveConfirmations(sendTxHash: string) {
    return withInterval( async () => {
      try {
  ...
  private static async getConfirmationsCount(swapTxHash: string): Promise<number> {
    try {
  ...
  private static async getSubmissionId(swapTxHash: string): Promise<string | null> {
    try {
      const result = await axios({
  ...
  private static async getFullSubmissionInfo(swapTxHash: string): Promise<FullSubmissionInfo | null> {
    try {
      const result = await axios({
        url: this.config.api + 'Transactions/GetFullSubmissionInfo',
        method: 'get',
